...
**Сделать:**
- `createRedis(url)` (ioredis, `maxRetriesPerRequest: null`), фабрика `Queue` по реестру `QUEUE_NAMES` из `@sf/core` (ленивое создание, кэш); три служебные очереди (`system.dlq`, `system.heartbeat`, `system.smoke`) лежат в самом `QUEUE_NAMES` (`packages/core/src/domain/queues.ts`), не в реестре воркера.
...
- Условие, на котором PM принял «все очереди реестра засеяны `true`» (26 на дату решения, 29 после этой задачи): каждый эпик, добавляющий обработчик платной или публикующей очереди, приносит свой предохранитель **в той же задаче**, не позже (переключатели очередей предохранителем не считаются). Для E0-08 это значит: обработчиков платных очередей здесь не заводить, а `system.*` — бесплатные.
...
- `Worker` на каждую очередь, у которой есть процессор в реестре `src/jobs/index.ts`, с `concurrency` из env; `Queue` открывается по всем именам `QUEUE_NAMES` (нужны переключателям и cron-реестру); очередь без процессора копит джобы в `waiting` (текущие пустые файлы — под будущие эпики).
...
**Сделать:**
- `infra/docker/api.Dockerfile`, `worker.Dockerfile` (multi-stage: `pnpm fetch` → `pnpm install --frozen-lockfile --prod=false` → `pnpm deploy --filter <app>` → runtime `node:22-alpine` с `tsx`, решение 1), `web.Dockerfile` (Next standalone, `NEXT_OUTPUT_STANDALONE=1`). CMD образа воркера — `node --import tsx src/index.ts` (тот же запуск, что у `pnpm --filter @sf/worker dev` и у `entry-point.int.test.ts`): любой родительский процесс перед воркером забирает сигнал себе и убивает активную задачу раньше дренажа.
- Compose: сервисы `migrate` (одноразовый, `depends_on: postgres healthy`), `api`, `worker`, `web` (`depends_on: migrate completed`), `env_file: .env`, volume `media:/data/media`; наружу только web `:3000` (api — только внутри сети). У сервиса `worker` — `stop_grace_period: 30s`: дедлайн graceful shutdown воркера — 25 с (`apps/worker/src/lib/shutdown.ts`), а `docker stop` по умолчанию шлёт SIGKILL через 10 с и убивает контейнер посреди дренажа. У api, worker и web объявить `NODE_ENV=production` явно (`environment:`), не полагаясь на fail-safe дефолт схемы: решение от 06.09 (E0-03) записывает дефолт только в `env`, а `process.env.NODE_ENV` остаётся пустым, и сторонние библиотеки уходят в dev-ветку.
