...
7. **Сборка — утром по TZ оператора, с потолком.** Cron `inbox.build` в `app_setting.inbox.build_time` (06:00, TZ `app_setting.timezone`), максимум `inbox.max_cards_per_day` (15) новых карточек; `jobId = inbox.build/<date>`, ручной «Пересобрать» — `inbox.build/<date>/<requestedAtMs>` (момент нажатия в epoch ms: ISO-8601 содержит `:`, который `jobId()` отвергает). Каждая карточка — отдельная попытка: ошибка LLM на одной не отменяет остальные.
