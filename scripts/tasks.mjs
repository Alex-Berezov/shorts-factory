...
const EPIC_ORDER = ["E0", "E12", "E1", "E2", "E3", "E4", "E5", "E6", "E7", "E8", "E9", "E10", "E11", "E13"];
const EPIC_NAMES = {
...
const save = (tasks) => writeFileSync(TASKS_PATH, `${JSON.stringify(tasks, null, 2)}\n`, "utf8");
...
const byOrder = (a, b) => epicIdx(a.epic) - epicIdx(b.epic) || a.id.localeCompare(b.id, "en", { numeric: true });

// ---------- queries ----------
const find = (tasks, id) => tasks.find((t) => t.id === id.toUpperCase()) ?? die(`unknown task ${id}`);
const depsDone = (tasks, t) => t.deps.every((d) => (tasks.find((x) => x.id === d) ?? {}).status === "done");
const available = (tasks) => tasks.filter((t) => t.status === "todo" && depsDone(tasks, t)).sort(byOrder);
const pendingDeps = (tasks, t) => t.deps.filter((d) => (tasks.find((x) => x.id === d) ?? {}).status !== "done");
...
function render(tasks, opts = {}) {
  ...
  const currentEpic = epics.find((e) => tasks.some((t) => t.epic === e && t.status !== "done")) ?? null;
  ...
  L.push("## Сводка");
  L.push("");
  L.push(`- **Прогресс:** ${bar(doneT.length, tasks.length, 30)} задач, ${fmtDays(days(doneT))} / ${fmtDays(days(tasks))} оценочных дней.`);
  L.push(`- **Текущий эпик:** ${currentEpic ? `${currentEpic} — ${EPIC_NAMES[currentEpic]}` : "все эпики завершены 🎉"}`);
  L.push(`- **В работе:** ${inProg.length ? inProg.map((t) => `${link(t)} ${t.title}`).join("; ") : "ничего — возьмите задачу из «Следующие»"}`);
  L.push(`- **Заблокировано:** ${blocked.length ? blocked.map((t) => `${link(t)} (${t.blocked_reason ?? "?"})`).join("; ") : "нет"}`);
  L.push("");
  ...
  if (inProg.length) {
    ...
    L.push("|---|---|---|---|");
    for (const t of inProg) L.push(`| ${link(t)} | ${cell(t.title)} | ${cell(t.estimate)} | ${t.started_at ?? ""} |`);
  ...
  if (next.length) {
    ...
    L.push("|---|---|---|---|");
    for (const t of next) L.push(`| ${link(t)} | ${cell(t.title)} | ${cell(t.estimate)} | ${t.epic} |`);
    L.push("");
    L.push(`Взять в работу: \`node scripts/tasks.mjs start ${next[0].id}\``);
  } else L.push("_Нет доступных задач: всё либо сделано, либо ждёт зависимостей._");
  L.push("");
  if (blocked.length) {
    ...
    L.push("|---|---|---|");
    for (const t of blocked) L.push(`| ${link(t)} | ${cell(t.title)} | ${cell(t.blocked_reason)} |`);
    L.push("");
  }
  ...
  for (const e of epics) {
    ...
    const st = d.length === all.length ? "✅" : all.some((t) => t.status !== "todo") ? "🔄" : "⬜";
    L.push(`| ${st} | **${e}** ${EPIC_NAMES[e]} | ${bar(d.length, all.length, 12)} | ${fmtDays(days(d))} / ${fmtDays(days(all))} | [${all[0].doc}](${all[0].doc}) |`);
  }
  L.push("");
  L.push("Легенда статусов: ⬜ не начато · 🔄 в работе / эпик начат · ⛔ заблокировано · ✅ сделано.");
  L.push("");
  L.push("## Все задачи");
  L.push("");
  for (const e of epics) {
    ...
    for (const t of all) {
      const deps = t.deps.length
        ? t.deps.map((d) => `${ICON[(tasks.find((x) => x.id === d) ?? {}).status ?? "todo"]}${d}`).join(" ")
        : "—";
      L.push(`| ${ICON[t.status]} | ${link(t)} | ${cell(t.title)} | ${cell(t.estimate)} | ${deps} | ${t.started_at ?? ""} | ${t.done_at ?? ""} |`);
    }
    L.push("");
  }
  ...
  L.push('node scripts/tasks.mjs done E0-01 "коротко что сделано"   # закрыть (+ запись в WORKLOG, перерисовка статуса)');
  L.push('node scripts/tasks.mjs block E0-05 "ждём ключ Gemini"     # заблокировать');
  ...
  if (opts.check) {
    ...
    const strip = (t) => t.split("\n").filter((l) => !l.startsWith("_Сгенерировано")).join("\n");
    let cur = "";
    try {
    ...
    if (strip(cur) !== strip(out)) {
      console.error("docs/00_STATUS.md is stale: run `node scripts/tasks.mjs render`");
      process.exit(1);
    }
...
switch (cmd) {
  ...
  case "next": {
    ...
    if (inProg.length) {
      ...
      for (const t of inProg) console.log(`  🔄 ${t.id}  ${t.title}  (${t.estimate})  → docs/${t.doc}`);
    }
    const av = available(tasks).slice(0, n);
    console.log(av.length ? "Next available (deps done):" : "Nothing available.");
    for (const t of av) console.log(`  ⬜ ${t.id}  ${t.title}  (${t.estimate})  → docs/${t.doc}`);
    break;
  }
  ...
  case "list": {
    ...
    for (const t of rows) console.log(`${ICON[t.status]} ${t.id.padEnd(7)} ${t.title}  (${t.estimate})`);
  ...
  case "show": {
    ...
    console.log(pend.length ? `pending deps: ${pend.join(", ")}` : "all deps done");
    break;
  }
  ...
  case "start": {
    const t = find(tasks, args[0] ?? die("task id required"));
    const pend = pendingDeps(tasks, t);
    if (pend.length && !args.includes("--force")) die(`deps not done: ${pend.join(", ")} (use --force to override)`);
  ...
  case "done": {
    ...
    delete t.blocked_reason;
    save(tasks);
    worklog(`- ${today()} ✅ **${t.id}** ${t.title}${note ? ` — ${note}` : ""}`);
    render(tasks);
  ...
  case "add": {
    ...
    const nums = [...estimate.matchAll(/\d+(?:\.\d+)?/g)].map((m) => Number(m[0]));
    tasks.push({
      id,
      epic,
      title,
      ...
      deps: (opt("--deps") ?? "").split(",").map((s) => s.trim().toUpperCase()).filter(Boolean),
  ...
  default:
    die(`unknown command ${cmd}. Valid: ${["next", "start", "done", "block", "unblock", "todo", "show", "list", "render"].join(", ")}; statuses: ${STATUSES.join(", ")}`);
}
